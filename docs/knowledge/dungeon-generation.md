...
Base profile **DP_BASIC** (`Globals.c:959-968`):
```
{Cross=2, SymCross=1, Small=1, Circular=1, Chunky=7, Cave=1, Cavern=0, Entrance=0}
corridorChance=10
```

### Depth Adjustment (`Architect.c:2425-2449`)
```
...
typedef struct dungeonProfile {
    short roomFrequencies[ROOM_TYPE_COUNT];  // 8 room types
    short corridorChance;                     // 0-100
} dungeonProfile;
```

### Profiles (`Globals.c:959-977`)
| Profile               | Use              | Room Weights       | Corridor |
|-----------------------|------------------|--------------------|----------|
| DP_BASIC              | Standard dungeon | 2,1,1,1,7,1,0,0    | 10       |
| DP_BASIC_FIRST_ROOM   | First room       | 10,0,0,3,7,10,10,0 | 0        |
| DP_GOBLIN_WARREN      | Goblin machine   | 0,0,1,0,0,0,0,0    | 0        |
| DP_SENTINEL_SANCTUARY | Sentinel machine | 0,5,0,1,0,0,0,0    | 0        |
...
| `Architect.c` | Room designs: `designCrossRoom()` (2023), `designCircularRoom()` (2073), `designChunkyRoom()` (2091), `designCavern()` (1971), `designEntranceRoom()` (2005) |
