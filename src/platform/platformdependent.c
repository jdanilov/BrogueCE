...
static fileEntry *addfile(struct filelist *list, const char *name) {
    ...
    list->files[list->nfiles].path = (char *)(uintptr_t)list->nextname; // don't look at them until they are transferred out
...
static fileEntry *commitFilelist(struct filelist *list, char **namebuffer) {
    ...
    */
    for (i=0; i < list->nfiles; i++) {
        list->files[i].path = list->names + (uintptr_t)list->files[i].path;
    }
